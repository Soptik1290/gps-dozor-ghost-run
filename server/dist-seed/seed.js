...
require("dotenv/config"); // Must be first — loads DATABASE_URL for Prisma v7
...
function generatePragueRoute(startLat, startLng, startTime, durationSeconds, avgSpeedKmh) {
    ...
    for (let i = 0; i < totalPoints; i++) {
        ...
        const speedFactor = t < 0.1 ? t / 0.1 : // acceleration
...
async function main() {
    ...
    const vehicles = await prisma.vehicle.createManyAndReturn({
        data: [
            { name: 'BMW 740i', plate: '3A2 1000', status: prisma_1.VehicleStatus.ACTIVE, groupCode: 'EXEC' },
            { name: 'Škoda Octavia RS', plate: '1AB 2345', status: prisma_1.VehicleStatus.ACTIVE, groupCode: 'FLEET' },
            { name: 'Volkswagen Transporter', plate: '5C3 7890', status: prisma_1.VehicleStatus.ACTIVE, groupCode: 'FLEET' },
            { name: 'Audi A6', plate: '2D4 5678', status: prisma_1.VehicleStatus.ACTIVE, groupCode: 'EXEC' },
            { name: 'Mercedes Sprinter', plate: '4E5 4321', status: prisma_1.VehicleStatus.SERVICE, groupCode: 'FLEET' },
        ],
    });
    ...
    for (let day = 0; day < 5; day++) {
        ...
        const logs = generatePragueRoute(PRAGUE_LAT - 0.05 + day * 0.01, PRAGUE_LNG - 0.03 + day * 0.01, tripStart, Math.floor(duration), avgSpd);
    ...
    for (let day = 0; day < 3; day++) {
        ...
        const logs = generatePragueRoute(PRAGUE_LAT + 0.08 + day * 0.005, PRAGUE_LNG + 0.04 + day * 0.005, tripStart, Math.floor(duration), avgSpd);
        const trip = await prisma.trip.create({
            data: {
